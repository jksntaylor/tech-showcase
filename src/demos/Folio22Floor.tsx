...
void main() {
  ...
  float uFreqSmall = 3.;
  float uSpeedSmall = 0.2;
  float uElevationSmall = 0.3;

  float uFreqBigX = 0.05;
  float uFreqBigY = 0.07;
  float uSpeedBig = 1.1;
  float uElevationBig = 0.01;

  vec4 modelPosition = modelMatrix * vec4(position, 1.0);

  // Elevation
  float elevation = sin(modelPosition.x * uFreqBigX + time * uSpeedBig) * sin(modelPosition.z * uFreqBigY + time * uSpeedBig) * uElevationBig;

  for(float i = 1.0; i <= 1.; i++) {
    elevation -= abs(cnoise(vec3(modelPosition.zx * uFreqSmall * i, time * uSpeedSmall)) * uElevationSmall / i);
  }

  vHeight += elevation;

  vec3 newPosition = position + normal * 10. * vHeight;
...
void main() {
  ...
  vec3 peakColor = vec3(0.259, 0.961, 0.961);
...
const Sketch = () => {
  ...
  return <mesh rotation={[-1.5, 0, 0]}>
    <planeGeometry args={[50, 50, 1024, 1024]} />
